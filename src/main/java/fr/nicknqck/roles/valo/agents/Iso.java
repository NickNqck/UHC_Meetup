...
public class Iso extends RoleBase {
    ...
    public void RoleGiven(GameState gameState) {
        ...
        desc.setItems(new TripleMap<>(
                new HoverEvent(HoverEvent.Action.SHOW_TEXT, new BaseComponent[]{new TextComponent(
                        "Le nombre de coup que vous infligerez, vous les stoquerez pendant les 15s qui suivent l'activation de la Protection. Après la fin du timer vous pourrez ésquiver le nombre de coups stoqué"
                )}),
                "Protection couplé",
                60
        ), new TripleMap<>(
                new HoverEvent(HoverEvent.Action.SHOW_TEXT, new BaseComponent[]{new TextComponent(
                        "Vous permet de tirer"
                )}),
                "Barrière",
                60
    ...
    public void Update(GameState gameState) {
        ...
        if (cdProtection >= 0) {
            ...
            if (cdProtection >= 60 * 7) {
                sendCustomActionBar(owner, "Coup accumulé:§c " + stackedCoup + "§7 |§f Temp restant pour l'accumulation: §c" + StringUtils.secondsTowardsBeautiful((60 * 7 + 15) - cdProtection) + "§7/§c15s");
